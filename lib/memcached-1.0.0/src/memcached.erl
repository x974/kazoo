...
%%====================================================================
%% @doc Delete a key from memcached
%% @spec version(memcached_connection()) ->
%%         [string()]
-spec(version/1::(memcached_connection()) ->
         [string()]).
version({host, Host, port, Port}) ->
  {ok, Socket} = gen_tcp:connect(Host, Port, [binary, {active, false}]),
  Reply = process_version(Socket),
  gen_tcp:close(Socket),
  Reply;
version({socket, Socket}) ->
  process_version(Socket).
...
%% Send version and handle the response
process_version(Socket) ->
  Line = <<"version\r\n">>,
  ok = gen_tcp:send(Socket, Line),
  {ok, Response} = gen_tcp:recv(Socket, 0),
