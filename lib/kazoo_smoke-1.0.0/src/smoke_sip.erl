...
extract_method(SipReq, Buffer0) ->
    ...
    case is_known_method(M0) of
        {true, M1} -> {smoke_sip_req:set_method(SipReq, M1), Buffer1};
        false -> {error, 501}
    end.
...
extract_request_uri(SipReq, Buffer) ->
    case extract_sip_uri(Buffer) of
        {error, _}=E -> E;
        {SipUri, Buffer1} -> {smoke_sip_req:set_request_uri(SipReq, SipUri), Buffer1}
    end.
...
extract_sip_params(<<"transport=", Buffer/binary>>, #sip_uri_params{transport=undefined}=Params) ->
    ...
    case is_known_transport(V) of
        ...
        false -> io:format("400 transport~n"), {error, 400}
    end;
...
extract_sip_params(<<"method=", Buffer/binary>>, #sip_uri_params{method=undefined}=Params) ->
    ...
    case is_known_method(V) of
        ...
        false -> io:format("400 method: ~p~n", [V]), {error, 400}
    end;
extract_sip_params(Buffer, #sip_uri_params{other=Other}=Params) ->
    ...
    case props:get_value(Key, Other) of
        ...
        _V ->
            ...
            io:format("400 param exists: ~p~n", [_V]),
            {error, 400}
    end.
