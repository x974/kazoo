...
init([]) ->
    {ok, { {one_for_one, 5, 10}, [
                                  ?CACHE(milliwatt_cache) %% generally, we create a local cache process per whapps
                                  ,?CHILD(milliwatt_listener, worker) %% the listener, we always want this running
                                 ]} }.
