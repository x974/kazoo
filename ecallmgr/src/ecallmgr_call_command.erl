    ...
    case wapi_dialplan:call_pickup_v(JObj) of
        ...
        true ->
            ...
            _ = case wh_json:is_true(<<"Park-After-Pickup">>, JObj, false) of
                    false -> export(Node, Target, <<"park_after_bridge=false">>);
                    true -> export(Node, Target, <<"park_after_bridge=true">>)
                end,

            _ = case wh_json:is_true(<<"Continue-On-Fail">>, JObj, true) of
                    false -> export(Node, Target, <<"continue_on_fail=false">>);
                    true -> export(Node, Target, <<"continue_on_fail=true">>)
                end,

            _ = case wh_json:is_true(<<"Continue-On-Cancel">>, JObj, true) of
                    false -> export(Node, Target, <<"continue_on_cancel=false">>);
                    true -> export(Node, Target, <<"continue_on_cancel=true">>)
                end,

            _ = export(Node, Target, <<"failure_causes=NORMAL_CLEARING,ORIGINATOR_CANCEL,CRASH">>),
